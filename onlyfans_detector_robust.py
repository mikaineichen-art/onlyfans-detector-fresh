#!/usr/bin/env python3
"""
Robust Hybrid OnlyFans Detector for n8n Integration
Phase 1: Fast HTTP detection (1-2 seconds)
Phase 2: Deep Puppeteer investigation (5-15 seconds) - only if Phase 1 finds nothing
Phase 3: Enhanced HTTP detection (3-5 seconds) - automatic fallback if Phase 2 fails
"""
...
class RobustOnlyFansDetector:
    """Robust detector with automatic fallbacks"""
    ...
    async def detect_onlyfans(self, bio_link: str) -> Dict:
        """Main detection method - robust hybrid approach with fallbacks"""
        self.results = {
            "has_onlyfans": False,
            "onlyfans_urls": [],
            "detection_method": None,
        ...
        try:
            ...
            # Phase 2: Deep investigation (5-15 seconds) - only if Phase 1 found nothing
            ...
            # Phase 3: Enhanced HTTP detection (3-5 seconds) - automatic fallback
            self.results["debug_info"].append("Phase 2 failed - starting Phase 3: Enhanced HTTP detection")
            self.results["fallback_used"] = True
            if await self._phase3_enhanced_http(bio_link):
                self.results["phase_used"] = "phase3_enhanced_http"
                self.results["debug_info"].append("Phase 3 successful - OnlyFans found via enhanced HTTP")
                return self.results
    ...
    async def _phase2_deep_investigation(self, bio_link: str) -> bool:
        """Phase 2: Deep Puppeteer investigation (5-15 seconds) with timeout protection"""
        try:
            ...
            try:
                ...
                await page.goto(bio_link, {'waitUntil': 'domcontentloaded', 'timeout': 10000})
                await page.waitFor(2000)  # Wait for JS to load
    ...
    async def _phase3_enhanced_http(self, bio_link: str) -> bool:
        """Phase 3: Enhanced HTTP detection with sophisticated parsing (3-5 seconds)"""
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                # Get the page with different user agents
                headers_list = [
            ...
            self.results["errors"].append(f"Phase 3 enhanced HTTP failed: {str(e)}")
            ...
            for pattern in js_patterns:
                ...
                for match in matches:
                    if 'onlyfans.com' in match and '/files' not in match and '/public' not in match:
                        ...
                        self.results["detection_method"] = "phase3_js_variable_scan"
                        self.results["debug_info"].append(f"Phase 3: Found OnlyFans via JS variable: {match}")
                        return True
    ...
    async def _enhanced_redirect_analysis(self, content: str, base_url: str, client: httpx.AsyncClient) -> bool:
        """Enhanced redirect chain analysis for Phase 3"""
        try:
            ...
            # Check first 15 links (more than Phase 1)
            for link in all_links[:15]:
                ...
                try:
                    ...
                    if OF_REGEX.search(final_url) and '/files' not in final_url and '/public' not in final_url:
                        self.results["has_onlyfans"] = True
                        self.results["onlyfans_urls"] = [final_url]
                        self.results["detection_method"] = "phase3_enhanced_redirect"
                        self.results["debug_info"].append(f"Phase 3: Found OnlyFans via enhanced redirect: {link} → {final_url}")
                        return True
    ...
    async def _follow_redirects_enhanced(self, client: httpx.AsyncClient, url: str, max_redirects: int = 5) -> Tuple[str, List[str]]:
        """Enhanced redirect following for Phase 3"""
        chain = [url]
        current = url
...
async def detect_onlyfans_in_bio_link(bio_link: str, headless: bool = True) -> Dict:
    ...
    detector = RobustOnlyFansDetector(headless=headless)
    return await detector.detect_onlyfans(bio_link)

def main():
    """Command line interface for n8n integration"""
    if len(sys.argv) != 2:
        print("Usage: python onlyfans_detector_robust.py <bio_link>")
        print("Example: python onlyfans_detector_robust.py 'https://link.me/username'")
        sys.exit(1)
